...
def _enrich_extraction(row: dict, conn) -> dict:
    ...
    work = conn.execute(
        "SELECT title, year, doc_type, primary_doc_type, read_status, "
...
def list_extractions(
    ...
    try:
        ...
        rows = conn.execute(f"""
            ...
            JOIN works w ON w.id = ce.work_id
            WHERE {where_sql}
            ORDER BY ce.ambiguity_score DESC, ce.created_at DESC
            LIMIT ? OFFSET ?
...
def review_extraction(ext_id: str, body: ExtractionReview):
    ...
    try:
        ...
        conn.execute(
            ...
            "reviewed_at = ?, extracted_json = ?, updated_at = ? WHERE id = ?",
            (body.review_status, body.review_note, now,
             json.dumps(extracted, ensure_ascii=False), now, ext_id),
        )
        conn.commit()
