...
- 前端 `MetadataReview.vue` 在每个可重抽字段旁新增「重抽」和「复制」按钮；字段使用 canonical key（如 `publication_date`、`contributors`），后端兼容旧别名 `date` / `institutions`。
...
- 新增 `api/metadata_template.py`：统一管理 `templates/templates.json` 路径、内置字段、用户自定义合并、字段校验、prompt 生成、rerun 字段白名单和旧字段别名。
- 新增 `templates/templates.json` baseline（metadata v1.1），让元数据模板成为可审查、可 diff、可交接的项目资产。
...
- 2026-07-18 主线阶段一·出口侧最小闭环：新增 BibTeX/RIS 引用导出与综述矩阵 CSV（`api/export_format.py` + `api/routes/export.py` 三端点，Works 页导出弹窗）；数据源严格限定"元数据已批准且未隔离"；两轮模型独立审核（`docs/superpowers/reviews/2026-07-18-export-review.md`），8+2 项 finding 全部修复；方案见 `docs/_archive/superpowers/plans/2026-07-18-export-minimal-loop.md`。
- 2026-07-18 主线阶段二·驾驶舱与流程接力：`/api/pipeline/stats` 扩展 intake/backlog 三指标并修正 inbox 递归计数；Dashboard 升级为待办面板（流程待办+积压补审+建议下一步）；useNextStep 接力提示接入摄入/抽取完成点；分类前置提示；审核页自动下一条。两轮独立审核（`docs/superpowers/reviews/2026-07-18-cockpit-review.md`），8 项 finding 修复（含 useNextStep message API 高危误用、待去重字段名错误）。方案见 `docs/_archive/superpowers/plans/2026-07-18-cockpit-relay.md`。随后 2f336fe 补 backlog.*_all 全量口径并在面板标注口径差异。
- 2026-08-29 UX-002 解析断点打通（PDF → markdown + 图片）：`pymupdf_client` 补栅格图抽取（`get_image_info(xrefs=True)` + images/ 契约 + md 引用）；`router.route_and_parse` 增加 `backend` 覆盖与解析产物清理；`POST /api/parse/trigger` 接线 `backend`/`force`（重解析不限 pending）；CLI `literature_batch_parse.py` 增加 `--backend/--force/--work-ids`；新增 `GET /api/files/{work_id}/images/{name}` 图片端点（文件名白名单 + works 边界校验）；WorkDetail 内容预览升级为纯 JS markdown 渲染（`web/src/utils/markdown.js`，无新依赖）+ 图片内联 + 「重新解析」后端下拉。新增 24 项测试，全量 592 passed，build/healthcheck 通过。详见 `USER_ISSUES.md` 已解决 UX-002。
- 2026-08-29 UX-007 输出层合并接线（方案C）：`detail_result/merged_md.py` 把原服务逐字节一致的 `merge.merged()` 接回链路——cloud vlm 成功后自动产出旁路资产 `detail.json`（unified_merge_schema v4）+ `content.merged.md`（MinerU 结构主体 + PyMuPDF 字形样式，公式/图表/算法/脚注从 v2 原始节点回填）；新增 `/api/files/{id}/merged`、`/detail` 端点与 WorkDetail 三态预览（合并版/原版/原文 + 诊断 JSON 链接）；`scripts/literature_merge_detail.py` 离线回填存量 8 篇。原始 content.md 等产物零改动。新增 14 项测试，全量 606 passed。
- 2026-08-31 双路合并解析成为默认路由 + 15 篇批量重解析：`route_and_parse` 的 auto 语义由 D13 二元路由改为「cloud vlm 优先（成功即生成合并旁路资产）→ 失败回退 PyMuPDF（带质检）」；CLI 新增 `--source-file-ids`（多源 work 精准指定副本）；`files.py` 三处查询改按 `finished_at DESC` 取最新 run（修复多源 work 中 v2 合并资产被旧 run 遮蔽的 404）。按用户指令对 15 篇安全评测文献强制 vlm 重解析并全部生成合并视图（ECD 此前已有，共 16 篇可读合并版）；ForesightSafety-SAGE 按 v2 副本 SF-7a2f51938b5c-00190 解析。全量 610 passed。
