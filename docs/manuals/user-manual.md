...
1. 新文献进入：本地 PDF 走 `/ingest` 收件箱的「上传 PDF」按钮（多选，存入 _inbox 并自动摄入），或手工把 PDF 放进 `<library_root>/_inbox/` 后在该页「重新扫描」确认；开放网络检索走 `/topics` 或 `/discovery`。
2. 候选审核：发现检索命中先在 `/discovery` 审核，采集候选再在 `/intake` 审核。
3. 正式入库：批准后的候选进入 `works`，并产生 pending 解析任务。
4. 解析：在 `/pipeline` 或单篇 `/works/:id` 触发解析；解析结果不满意可在单篇页用「重新解析」指定后端重跑。
...
- 默认解析（2026-08-31 起为**双路合并**）：云 VLM 优先，成功后自动生成合并视图
  （MinerU 结构 + PyMuPDF 字形样式 + 完整版面图表）；云服务不可用/无额度时自动
  回退 PyMuPDF 本地直抽（文本层 + 栅格图，无合并视图），流水线不断。
- 页面「解析状态」行有后端选择下拉与「重新解析」按钮。「自动」即双路合并默认；
  「强制云 VLM」失败不回退（必须拿到云端产物时用）；「强制 PyMuPDF」秒级本地重抽
  （免费，但矢量图表/扫描件不覆盖）。
- 内容预览三态切换：
  - **合并版**（默认，仅云 VLM 解析有）：MinerU 结构为主体 + PyMuPDF 字形样式
    （粗体/斜体/行内公式）的合并视图，云 VLM 解析成功后自动生成；点「合并诊断
    JSON」可查看合并明细，用于溯源。
  - **原版**：content.md（MinerU full.md）渲染视图。
  - **原文**：content.md 原始文本。
  - 合并版与原版图片均内联显示；两者不一致时以原版为准，可对照排查。
- 存量云 VLM 文献可用 `python scripts\literature_merge_detail.py --execute`
  离线回填合并产物（不耗额度、不重新解析）。
