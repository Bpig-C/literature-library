...
- A. 元数据抽取模板固化
  ...
  - **当前下一步**：UX-004 字段级重抽前端化（智能 opencode + 笨模式复制 prompt）
  - 详见 `USER_ISSUES.md` UX-004 和 `FUTURE_WORK_PLAN.md` §A
- B. 分类规范与审核模板
...
2. **UX-004**：字段级重抽前端入口 —— 后端 CLI 就绪，需做 HTTP endpoint + UI 按钮
3. **UX-001**：批量触发流程管理页面 —— Dashboard 增强
4. **#11**：proposed_new 标签转正路径清晰化
5. **QA-001**：模板管理只完成 CRUD，尚未接入真实抽取链路
6. **QA-002**：候选 PDF 手动上传缺少生命周期保护
7. **QA-003**：收件箱上传缺少大小限制和同名保护
