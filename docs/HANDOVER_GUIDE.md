...
| 数据库 schema 谨慎变更 | 默认不改；若任务确需新增/调整 schema，必须先获得用户确认，并补迁移/回滚/fixture 测试 |
| 纯 JS | 不引入 TypeScript |
...
- A. 元数据抽取模板固化
  ...
  - **当前下一步建议**：元数据模板资产治理仍可继续；UX-002 图片丢失已于 2026-08-29 解决（PyMuPDF 栅格图抽取 + backend/force 重解析 + 图片端点与前端渲染）；下一候选为 #1 Discovery 本地模型自动执行器；分类词汇发布流程 QA-004 维持有意暂缓
...
| 编号 | 任务 | 优先级 | 状态 |
|------|------|--------|------|
| UX-001 | 批量触发流程管理入口 | 🔴 高 | ✅ 已完成（Pipeline 分阶段批量触发；Dashboard 有入口） |
| UX-002 | PyMuPDF 解析丢失图片/图表 | 🟡 中 | ✅ 已完成（2026-08-29，见 USER_ISSUES.md） |
| UX-007 | 输出层合并（MinerU 结构+PyMuPDF 样式）未接线 | 🟡 中 | ✅ 已完成（2026-08-29 方案C：旁路 detail.json + content.merged.md + 前端三态预览） |
...
2. **启动前后端**：后端推荐 `uv run python scripts\run_api.py`（默认 19527）；开发热重载可用 `uvicorn api.main:app --port 19527 --reload`；前端 `npm run dev`（端口 19528）
...
1. ~~**UX-002**：PyMuPDF 解析丢失图片~~ → ✅ 2026-08-29 已解决（图片抽取 + 重解析入口 + 前端渲染）
