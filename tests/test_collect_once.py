...
def test_collect_once_topic_plus_explicit_attributed_to_topic(temp_db, monkeypatch):
    """MINOR-1 回归：--topic T --ids A 时，A 的候选也归属 T（collection_topic_id=T）。

    契合「按主题发起一次采集」心智模型。固化语义防重构漂移。
    """
    import collector.collect as collect_mod
    import collector.topics as topics

    topics.create(name="Tcombo", description="d", explicit_ids=["2501.44444"],
                  axis_hint="risk_domain")
    tid = topics.list_topics()[-1]["id"]

    seen = {}

    def fake_explicit(ids, *, source_type="arxiv", collection_topic_id=None):
        seen.setdefault("topic_ids", []).append(collection_topic_id)
        return [{"id": f"C-{i}", "arxiv_id": v, "title": "t", "resolution": "pending"}
    ...
    collect_mod.collect_once(topic_id=tid, explicit_ids=["2501.55555"])
    # 合并后唯一一次 collect_explicit 调用，且 collection_topic_id == tid
    assert len(seen["topic_ids"]) == 1
    assert seen["topic_ids"][0] == tid
